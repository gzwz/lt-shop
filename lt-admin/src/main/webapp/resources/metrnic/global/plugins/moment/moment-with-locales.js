		...
		(
				...
				function() {
					...
					function valid__isValid(m) {
						if (m._isValid == null) {
							...
							m._isValid = !isNaN(m._d.getTime())
									...
									&& !flags.invalidMonth && !flags.nullInput
									&& !flags.invalidFormat
					...
					function addRegexToken(token, regex, strictRegex) {
						regexes[token] = typeof regex === 'function' ? regex
								: function(isStrict) {
					...
					function cloneWithOffset(input, model) {
						...
						return model._isUTC ? local__createLocal(input).zone(
								model._offset || 0) : local__createLocal(input)
								.local();
					}
					...
					function isDaylightSavingTimeShifted() {
						if (this._a) {
							var other = this._isUTC ? create_utc__createUTC(this._a)
									: local__createLocal(this._a);
							return this.isValid()
									&& compareArrays(this._a, other.toArray()) > 0;
						}
					...
					addParseToken([ 'S', 'SS', 'SSS', 'SSSS' ], function(input,
							array) {
					...
					function bubble() {
						...
						// Accurately convert days to years, assume start from
						// year 0.
						years = absFloor(daysToYears(days));
						days -= absFloor(yearsToDays(years));

						// 30 days to a month
						// TODO (iskren): Use anchor date (like 1st Jan) to
						// compute this.
					...
					function as(units) {
						...
						if (units === 'month' || units === 'year') {
							...
							days = this._days
									+ Math
											.round(yearsToDays(this._months / 12));
							switch (units) {
							case 'week':
					...
					function iso_string__toISOString() {
						...
						var s = iso_string__abs(this.seconds()
					...
					// ! moment.js
					// ! version : 2.10.3
					// ! authors : Tim Wood, Iskren Chernev, Moment.js
					// contributors
					// ! license : MIT
					// ! momentjs.com
					...
					var af = _moment__default
							.defineLocale(
									'af',
									{
										...
										week : {
											dow : 1, // Maandag is die eerste
											// dag van die week.
											doy : 4
										// Die week wat die 4de Januarie bevat
										// is die eerste week van die jaar.
										}
									});

					// ! moment.js locale configuration
					// ! locale : Moroccan Arabic (ar-ma)
					// ! author : ElFadili Yassine :
					// https://github.com/ElFadiliY
					// ! author : Abdel Said : https://github.com/abdelsaid

					var ar_ma = _moment__default
							.defineLocale(
									'ar-ma',
									{
					...
					}, ar__months = [ 'كانون الثاني يناير', 'شباط فبراير',
							...
							.defineLocale(
									'az',
									{
										...
										ordinal : function(number) {
											...
											var a = number % 10, b = number
													% 100 - a, c = number >= 100 ? 100
													: null;
											return number
													+ (az__suffixes[a]
															|| az__suffixes[b] || az__suffixes[c]);
										},
										week : {
											dow : 1, // Monday is the first
											// day of the week.
					...
					function be__relativeTimeWithPlural(number, withoutSuffix,
							key) {
						var format = {
					...
					var bn = _moment__default
							.defineLocale(
									'bn',
									{
										...
										// in different forms in various parts
										// of the world.
										// I have just generalized with most
										// common one used
										meridiem : function(hour, minute,
												isLower) {
											if (hour < 4) {
					...
					function relativeTimeWithMutation(number, withoutSuffix,
							key) {
						var format = {
							'mm' : 'munutenn',
							'MM' : 'miz',
					...
					function bs__translate(number, withoutSuffix, key) {
						...
						switch (key) {
						case 'm':
							return withoutSuffix ? 'jedna minuta'
									: 'jedne minute';
						case 'mm':
							if (number === 1) {
								result += 'minuta';
					...
					var cs__months = 'leden_únor_březen_duben_květen_červen_červenec_srpen_září_říjen_listopad_prosinec'
							.split('_'), cs__monthsShort = 'led_úno_bře_dub_kvě_čvn_čvc_srp_zář_říj_lis_pro'
							.split('_');
					...
					function cs__translate(number, withoutSuffix, key, isFuture) {
						...
						case 'h': // an hour / in an hour / an hour ago
							return withoutSuffix ? 'hodina'
									: (isFuture ? 'hodinu' : 'hodinou');
						case 'hh': // 9 hours / in 9 hours / 9 hours ago
							if (withoutSuffix || isFuture) {
								return result
										+ (cs__plural(number) ? 'hodiny'
												: 'hodin');
							} else {
					...
					var cs = _moment__default
							.defineLocale(
									'cs',
									{
										months : cs__months,
										monthsShort : cs__monthsShort,
										monthsParse : (function(months,
												monthsShort) {
											var i, _monthsParse = [];
											for (i = 0; i < 12; i++) {
					...
					var cy = _moment__default
							.defineLocale(
									'cy',
									{
										...
										// traditional ordinal numbers above 31
										// are not commonly used in colloquial
										// Welsh
										ordinal : function(number) {
					...
					function de_at__processRelativeTime(number, withoutSuffix,
							key, isFuture) {
						var format = {
					...
					function de__processRelativeTime(number, withoutSuffix,
							key, isFuture) {
						var format = {
							'm' : [ 'eine Minute', 'einer Minute' ],
							'h' : [ 'eine Stunde', 'einer Stunde' ],
							...
							.defineLocale(
									'el',
									{
										...
										months : function(momentToFormat,
												format) {
											if (/D/.test(format.substring(0,
													format.indexOf('MMMM')))) { // if
												// there
												// is a
												// day
												// number
					...
					// ! komento: Mi estas malcerta se mi korekte traktis
					// akuzativojn en tiu traduko.
					// ! Se ne, bonvolu korekti kaj avizi min por ke mi povas
					// lerni!

					var eo = _moment__default
							.defineLocale(
									'eo',
									{
					...
					var monthsShortDot = 'Ene._Feb._Mar._Abr._May._Jun._Jul._Ago._Sep._Oct._Nov._Dic.'
							.split('_'), es__monthsShort = 'Ene_Feb_Mar_Abr_May_Jun_Jul_Ago_Sep_Oct_Nov_Dic'
							.split('_');

					var es = _moment__default
							.defineLocale(
									'es',
									{
					...
					function et__processRelativeTime(number, withoutSuffix,
							key, isFuture) {
						var format = {
						...
						if (withoutSuffix) {
							return format[key][2] ? format[key][2]
									: format[key][1];
						}
						return isFuture ? format[key][0] : format[key][1];
					}

					var et = _moment__default
							.defineLocale(
									'et',
									{
					...
					// ! moment.js locale configuration
					// ! locale : faroese (fo)
					// ! author : Ragnar Johannesen :
					// https://github.com/ragnar123

					var fo = _moment__default
							.defineLocale(
									'fo',
									{
					...
					var fy__monthsShortWithDots = 'jan._feb._mrt._apr._mai_jun._jul._aug._sep._okt._nov._des.'
							.split('_'), fy__monthsShortWithoutDots = 'jan_feb_mrt_apr_mai_jun_jul_aug_sep_okt_nov_des'
							.split('_');

					var fy = _moment__default
							.defineLocale(
									'fy',
									{
										...
										monthsShort : function(m, format) {
											if (/-MMM-/.test(format)) {
												return fy__monthsShortWithoutDots[m
														.month()];
											} else {
												return fy__monthsShortWithDots[m
														.month()];
											}
										},
					...
					var hi = _moment__default
							.defineLocale(
									'hi',
									{
										...
										// Hindi notation for meridiems are
										// quite fuzzy in practice. While there
										// exists
					...
					var hr = _moment__default
							.defineLocale(
									'hr',
									{
										months : 'siječanj_veljača_ožujak_travanj_svibanj_lipanj_srpanj_kolovoz_rujan_listopad_studeni_prosinac'
												.split('_'),
					...
					function lb__processRelativeTime(number, withoutSuffix,
							key, isFuture) {
						var format = {
					...
					/**
					 * Returns true if the word before the given number loses
					 * the '-n' ending. e.g. 'an 10 Deeg' but 'a 5 Deeg'
					 * 
					 * @param number
					 *            {integer}
					 * @returns {boolean}
									...
									'lb',
									{
										...
										calendar : {
											...
											lastWeek : function() {
												...
												// 'Donneschdeg' (Thursday) due
												// to phonological rule
												switch (this.day()) {
												case 2:
												case 4:
					...
					var lt__units = {
						...
						'yy' : 'metai_metų_metus'
							...
							.split('_');
					function translateSeconds(number, withoutSuffix, key,
							isFuture) {
						if (withoutSuffix) {
							return 'kelios sekundės';
						} else {
							return isFuture ? 'kelių sekundžių'
									: 'kelias sekundes';
						}
					}
					function translateSingular(number, withoutSuffix, key,
							isFuture) {
					...
					/**
					 * @param withoutSuffix
					 *            boolean true = a length of time; false =
					...
					function lv__relativeTimeWithPlural(number, withoutSuffix,
							key) {
						return number
								+ ' '
								+ lv__format(lv__units[key], number,
										withoutSuffix);
					}
					function relativeTimeWithSingular(number, withoutSuffix,
							key) {
						return lv__format(lv__units[key], number, withoutSuffix);
					}
					function relativeSeconds(number, withoutSuffix) {
						return withoutSuffix ? 'dažas sekundes'
								: 'dažām sekundēm';
					}

					var lv = _moment__default
							.defineLocale(
									'lv',
									{
					...
					var me = _moment__default.defineLocale('me', {
						...
						monthsShort : [ 'jan.', 'feb.', 'mar.', 'apr.', 'maj',
								...
								'dec.' ],
						weekdays : [ 'nedjelja', 'ponedjeljak', 'utorak',
								'srijeda', 'četvrtak', 'petak', 'subota' ],
						weekdaysShort : [ 'ned.', 'pon.', 'uto.', 'sri.',
								'čet.', 'pet.', 'sub.' ],
					...
					var nl__monthsShortWithDots = 'jan._feb._mrt._apr._mei_jun._jul._aug._sep._okt._nov._dec.'
							.split('_'), nl__monthsShortWithoutDots = 'jan_feb_mrt_apr_mei_jun_jul_aug_sep_okt_nov_dec'
							.split('_');

					var nl = _moment__default
							.defineLocale(
									'nl',
									{
										...
										monthsShort : function(m, format) {
											if (/-MMM-/.test(format)) {
												return nl__monthsShortWithoutDots[m
														.month()];
											} else {
												return nl__monthsShortWithDots[m
														.month()];
											}
										},
					...
					function pl__translate(number, withoutSuffix, key) {
						...
						case 'mm':
							return result
									+ (pl__plural(number) ? 'minuty' : 'minut');
						case 'h':
							return withoutSuffix ? 'godzina' : 'godzinę';
						case 'hh':
							return result
									+ (pl__plural(number) ? 'godziny'
											: 'godzin');
						case 'MM':
							return result
									+ (pl__plural(number) ? 'miesiące'
											: 'miesięcy');
						case 'yy':
							return result
									+ (pl__plural(number) ? 'lata' : 'lat');
						}
					}

					var pl = _moment__default
							.defineLocale(
									'pl',
									{
										months : function(momentToFormat,
												format) {
											if (format === '') {
												// Hack: if format empty we know
												// this is used to generate
												// RegExp by moment. Give then
												// back both valid forms of
												// months
												// in RegExp ready format.
												return '('
														+ monthsSubjective[momentToFormat
																.month()]
														+ '|'
														+ monthsNominative[momentToFormat
																.month()] + ')';
					...
					function ro__relativeTimeWithPlural(number, withoutSuffix,
							key) {
						var format = {
							'mm' : 'minute',
							'hh' : 'ore',
					...
					function ru__relativeTimeWithPlural(number, withoutSuffix,
							key) {
						var format = {
					...
					var sk__months = 'január_február_marec_apríl_máj_jún_júl_august_september_október_november_december'
							.split('_'), sk__monthsShort = 'jan_feb_mar_apr_máj_jún_júl_aug_sep_okt_nov_dec'
							.split('_');
					...
					function sk__translate(number, withoutSuffix, key, isFuture) {
						...
						case 's': // a few seconds / in a few seconds / a few
							...
							return (withoutSuffix || isFuture) ? 'pár sekúnd'
									: 'pár sekundami';
						case 'm': // a minute / in a minute / a minute ago
							return withoutSuffix ? 'minúta'
									: (isFuture ? 'minútu' : 'minútou');
						case 'mm': // 9 minutes / in 9 minutes / 9 minutes ago
							if (withoutSuffix || isFuture) {
								return result
										+ (sk__plural(number) ? 'minúty'
												: 'minút');
							} else {
					...
					function sl__processRelativeTime(number, withoutSuffix,
							key, isFuture) {
						var result = number + ' ';
						switch (key) {
						case 's':
					...
					// ! moment.js locale configuration
					// ! locale : Morocco Central Atlas Tamaziɣt in Latin
					// (tzm-latn)
					// ! author : Abdel Said : https://github.com/abdelsaid

					var tzm_latn = _moment__default
							.defineLocale(
									'tzm-latn',
									{
					...
					function uk__relativeTimeWithPlural(number, withoutSuffix,
							key) {
						var format = {
					...
					var uk = _moment__default
							.defineLocale(
									'uk',
									{
										...
										// M. E.: those two are virtually unused
										// but a user might want to implement
										// them for his/her website for some
										// reason
